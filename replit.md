...
- **Vertical vs. Horizontal Orientation:** Bipolar meta-dimension overlaying the 10-pole system. Vertical orientation captures transcendence, sacred hierarchy, ascent, discipline, and contempt for the common (Nietzsche, Evola, mystics). Horizontal orientation captures immanence, coalition, solidarity, lived experience, harm/safety, and in-group signaling (DEI, social justice, therapy-speak). Returns worldview_signature, key phrases, and dominant_mode classification. Text analysis only.
- **Verticality Radar (Stylometry):** Writing style analysis distinct from worldview orientation. Analyzes text for ego-pronoun rate (I/me density), metaphor density, sentence structure complexity, hypotaxis vs. parataxis, formality markers, and rhetorical strategies. Returns a verticality quotient (0-1 scale where 0=horizontal/narrative style, 1=vertical/logical style). Supports single-text analysis and two-text comparison with side-by-side results. Includes author matching (e.g., "Writes like Voltaire") and style signature profiles. Requires minimum 100 words per text. Uses fuchsia-to-pink gradient UI styling.
